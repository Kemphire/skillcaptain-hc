...
def product_retrive_by_specific_price_range(request):
    ...
    data = [
        {
            "name":prod.name,"price":prod.price,"description":prod.description,"category":prod.category,"created_at":prod.created_at
        }
        for prod in prods
    ]
    return JsonResponse(data,safe = False)
...
def product_added_withing_seven_days(request):
    prods = Product.objects.filter(created_at__lte = date.today(),created_at__gte = date.today() - timedelta(days=7))
